...
if not exists(select * from sys.triggers where [name] = 'ApplyRealmSecurityAfterInsertEntity')
	throw 50000 , 'Missing Dependency: SWSecurity.Realms', 0;

begin transaction;
	-- region [Assemble]
		create table [dbo].[Persons] (	  -- Pre-Insert Table Test
			First nvarchar(120) not null,
			Last nvarchar(120) not null,
		);
		insert into [dbo].[xw_PortalEntities] (PortalEntityName, DatabaseTableName, KeyColumnName, LastUpdated) values
			('Person', 'Persons', 'SecurityId', GETDATE()),
			('Widget', 'Widgets', 'SecurityId', GETDATE());
		create table [dbo].[Widgets] (	  -- Post-Insert Table Test
...
go
	...
	clean:
		alter security policy [SWSecurity].[RealmViewPolicy]
			...
			drop filter predicate on [dbo].[Widgets];
		drop table if exists [dbo].[Persons];
		drop table if exists [dbo].[Widgets];
		delete from [SWSecurity].[Realms]
			where [Name] = 'Realm T1' or [Name] = 'Realm T2';
		delete from [dbo].[xw_PortalEntities] where [PortalEntityName] = 'Person' or [PortalEntityName] = 'Widget';
